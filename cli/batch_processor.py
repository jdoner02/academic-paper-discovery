...
class BatchProcessor:
    ...
    def discover_configurations(self) -> Dict[str, Path]:
        """
        Discover all YAML configuration files in the config directory and subdirectories.

        Returns:
            Dictionary mapping config name to file path

        Educational Notes:
        - Uses pathlib recursive glob patterns for file discovery
        - Handles nested directory structures for organized configs
        - Creates hierarchical names from relative paths
        - Demonstrates defensive programming practices
        - Supports new organized folder structure (cybersecurity-dsc/, programming-principles-I-II/, etc.)
        """
        ...
        for config_file in self.config_dir.rglob("*.yaml"):
            # Create hierarchical name from relative path
            relative_path = config_file.relative_to(self.config_dir)
            # Use parent directory + filename (without extension) for organized naming
            if relative_path.parent != Path("."):
                config_name = f"{relative_path.parent.name}_{relative_path.stem}"
            else:
