...
class MDPIPaperRepository(PaperSourcePort):
                    ...
                    for record in records:
                        ...
                        if self._record_matches_query(record.metadata, query):
                            try:
                                ...
                                if (
                                    hasattr(record.header, "setSpec")
                                    and record.header.setSpec
                                ):
                                    journal_info = {}
                                    ...
                                    paper.source_metadata.source_specific_data.update(
                                        journal_info
                                    )
                                papers.append(paper)
                            except Exception as e:
                                self._logger.warning(
                                    f"Failed to convert record {record.header.identifier}: {e}"
                                )
                                continue
    ...
    def _record_matches_query(
        self, metadata: Dict[str, Any], query: SearchQuery
    ) -> bool:
        """
